...
-- 클러스터형 인덱스의 루트 페이지 => 리프페이지의 또는 인터널페이지의 참조 주소를 가지고 있음
-- 클러스터형 인덱스의 리브 페이지 => 실제 데이터가 저장이 되어있음

-- 보조 인덱스 테이블 생성
DROP TABLE IF EXISTS clustertbl;
CREATE TABLE clustertbl -- Secondary Table 약자
( userID  CHAR(8) ,
  name    VARCHAR(10) 
);
INSERT INTO clustertbl VALUES('LSG', '이승기');
...
-- 보조 인덱스의 리프 페이지에서는 인덱스 컬럼(useId) 기준으로 정렬되어 있음.
...
ALTER TABLE mixedtbl
	ADD CONSTRAINT UK_mixedtbl_userId
		UNIQUE (name);
