...
SELECT 
    title, length, if(length >= 120, '긴 영화', '짧은 영화') as '길이분류'
FROM
    film;
...
select title, length(title) '영화제목길이' from film;

-- 문제 4: rental 테이블에서 각 대여에 대한 대여 시작일(rental_date)과 반납 예정일(return_date) 사이의 일수를 계산하세요. 
SELECT 
    rental_date, return_date, datediff(return_date, rental_date) as 대여일수
FROM
    rental;

-- 문제 5: payment 테이블에서 모든 결제 금액을 달러 자리까지 반올림하여 조회하세요.
SELECT 
    payment_id, round(amount, 0)
FROM
    payment;
...
SELECT 
    customer_id, date_format(last_update, '%Y-%m-%d')
FROM
    customer;
